...
def main():
    ...
    I5_Index_df = get_i5_data(I5_xls)
    template_file = pd.ExcelFile(args.template, engine='openpyxl')
    run_info = pd.read_excel(template_file, 'Worksheet', skiprows=2, nrows=1, index_col=1, na_values=['NA'], engine='openpyxl')['Sequencing Run ID']
    run_name = run_info[0]
    run_proj = run_info.index.values[0]
    print("RUN INFO -------------------------", "Project: ", run_proj, "Run ID: ", run_name)
